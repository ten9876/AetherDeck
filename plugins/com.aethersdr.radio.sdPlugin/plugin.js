...
// Persisted via Stream Deck's global settings; updated when the property
// inspector saves new values via setGlobalSettings.
let globalSettings = { host: "localhost", port: 40001 };

function tciConnect() {
    try {
        const host = globalSettings.host || "localhost";
        const port = globalSettings.port || 40001;
        tciWs = new WebSocket(`ws://${host}:${port}`);
        tciWs.on("open", () => { console.log(`TCI connected (${host}:${port})`); });
...
function tciDisconnect() {
    if (tciReconnectTimer) { clearTimeout(tciReconnectTimer); tciReconnectTimer = null; }
    if (tciWs) { try { tciWs.close(); } catch (_) {} tciWs = null; }
}
...
sdWs.on("open", () => {
    ...
    // Pull persisted host/port (if any) before opening the TCI socket.
    sdWs.send(JSON.stringify({ event: "getGlobalSettings", context: sdUUID }));
...
sdWs.on("message", (data) => {
    ...
    if (msg.event === "keyDown") {
        ...
        const incoming = (msg.payload && msg.payload.settings) || {};
        const prevHost = globalSettings.host;
        const prevPort = globalSettings.port;
        globalSettings = {
            host: incoming.host || "localhost",
            port: parseInt(incoming.port, 10) || 40001,
        };
        // First receipt or change → (re)connect TCI with current settings.
        if (!tciWs || globalSettings.host !== prevHost || globalSettings.port !== prevPort) {
            tciDisconnect();
            tciConnect();
        }
    }
...
// TCI connect is deferred until didReceiveGlobalSettings arrives (or fails to,
// in which case we fall back to defaults after a short timeout).
setTimeout(() => { if (!tciWs) tciConnect(); }, 500);
